...
import { exportThreatsToCSV } from "../utils/export";
...
const DatabaseManager = () => {
  ...
  const [sqlQuery, setSqlQuery] = useState(
    "SELECT * FROM threat_logs WHERE severity = 'Critical' ORDER BY timestamp DESC LIMIT 10;"
  );
  ...
  useEffect(() => {
    ...
    const trafficSubscription = threatService.subscribeToNetworkTraffic(
      (newTraffic) => {
  ...
  const executeQuery = async () => {
    ...
    try {
      if (sqlQuery.trim().toLowerCase().startsWith("select")) {
        const tableName = sqlQuery.toLowerCase().includes("threat_logs")
          ? "threat_logs"
          : "network_traffic";
        ...
        const formattedResult = formatQueryResults(
          (data ?? []) as DatabaseRow[],
          tableName
        );
  ...
  const handleDatabaseAction = async (action: string) => {
    ...
    try {
      ...
      console.error(`Database operation failed for ${action}:`, error);
  ...
  const handleExportTable = async (tableName: string) => {
    try {
      let data: ThreatLog[] | NetworkTraffic[] = [];
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">
              Across {dbStats.totalTables} tables
            </p>
          </CardContent>
        </Card>
                                  ...
                                  <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-sm">
                                    ...
                                    <div>
                                      ...
                                      <Badge
                                        ...
                                        className="ml-1"
                                      >
                                        {traffic.classification ?? "Unknown"}
                                      </Badge>
                                    </div>
                                    ...
                                    <div>
                                      ...
                                      {traffic.source_ip ?? "Unknown"}:
                                      {traffic.source_port ?? "Unknown"}
                                    </div>
                                    ...
                                    <div>
                                      ...
                                      {traffic.destination_ip ?? "Unknown"}:
                                      {traffic.destination_port ?? "Unknown"}
                                    </div>
                                  </div>
                                </div>
                              ))}
              ...
              <div className="space-y-4">
                <div className="flex items-center space-x-2 mb-4">
                  ...
                  <Button
                    variant="outline"
                    onClick={() =>
                      handleDatabaseAction("Incremental Backup")
                    }
                  >
                    Incremental Backup
                  </Button>
